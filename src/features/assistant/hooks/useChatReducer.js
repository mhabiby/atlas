...
import { ask, debugAsk } from '../api/askClient';
...
export function useChatReducer(lang){
  ...
  const answerText = (res.answer && res.answer.trim()) ? res.answer.trim() : '';
  const content = answerText || (matches.length ? `Found ${matches.length} result(s).` : 'No answer returned.');
  dispatch({ type: 'RESPONSE', message: { id: newId(), role: 'assistant', content, time: Date.now(), meta: { matches, note: res.note, elapsedMs: res.elapsedMs } }, autoSelect: matches[0] });
    }).catch(()=>{
  ...
  const retrieveDebug = useCallback((text) => {
    const q = (text || '').trim();
    if(!q || state.loading) return;
    const message = { id: newId(), role: 'user', content: q, time: Date.now() };
    dispatch({ type: 'SEND', message });
    debugAsk(q).then(res => {
      if(res.error){
        dispatch({ type: 'ERROR', message: { id: newId(), role:'assistant', content: 'Debug retrieval error', time: Date.now(), meta:{ error:true } } });
        return;
      }
      const matches = res.matches || [];
      const content = `Debug retrieval: ${matches.length} match(es).`;
      dispatch({ type: 'RESPONSE', message: { id: newId(), role:'assistant', content, time: Date.now(), meta:{ matches, note: res.note, elapsedMs: res.elapsedMs, debug: true } } });
    });
  }, [state.loading]);
  ...
  return { state, send, selectDoctor, clearChat, retrieveDebug };
}
